(function calcNavHeight(){
  ...
  function markExternalLinks() {
    // console.log("asset index.js new func");
    let links = elems('a');
    if(links) {
      try {
        Array.from(links).forEach(function(link){
          ...
          if(link.href instanceof SVGAnimatedString){
            // console.log("is SVGAnimatedString:\t%s\n\tElements:\t%s", link.href,link.className);
            // url = new URL(link.href.animVal);
          } else {
            // console.log("is HREF:\t%s\n\tElements:\t%s", link.href,link.className);
  ...
  // function markExternalLinks() {
  //   console.log("asset index.js OG Function");
  //   let links = elems('a');
  //   if(links) {
  //     Array.from(links).forEach(function(link){
  //       let target, rel, blank, noopener, attr1, attr2, url, is_external;
