    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            for match in heading_pattern.finditer(f.read()):
                ...
                anchor = re.sub(r'[^\w\-]', '', anchor, flags=re.UNICODE)  # Remove invalid characters
...
def resolve_target_path(content_dir, source_file, target_link):
    ...
    # print("-----------------------------------------")
    # print(f"Resolving link: {target_link}")
                ...
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    for link, anchor in matches:
                        ...
                        if target_path and not os.path.exists(target_path):
                            issues.append(f"Broken link in {file_path}: {link} (expected file missing for Russian version)")
                        else:
