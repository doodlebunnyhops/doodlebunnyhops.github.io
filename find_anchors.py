...
def find_anchors_in_markdown(directory):
    """
    Scans all .ru.md files in the given directory, finds anchored links, and lists them.
    """
    anchor_pattern = re.compile(r'\[(.*?)\]\(#(.*?)\)')  # Matches [text](#anchor)
    results = {}
                ...
                for line_number, line in enumerate(content, 1):
                    ...
                    if matches:
                        ...
                        for match in matches:
                            results[file_path].append((line_number, match[0], match[1]))  # (line number, link text, anchor)
...
def display_results(results):
    ...
    for file, links in results.items():
        print(f"📂 **File:** {file}")
        for line_num, text, anchor in links:
            print(f"   - 📌 Line {line_num}: `[ {text} ](# {anchor} )`")
        print("\n" + "-" * 50)
